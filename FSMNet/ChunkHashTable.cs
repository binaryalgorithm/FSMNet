...
using static FSMNet.GPUController;
...
namespace FSMNet
{
    ...
    public class ChunkHashTable
    {
        ...
        public ChunkHashTable(float setMaxLoad = 0.5f)
        {
        ...
        public int Hash(int x, int y, int z)
        {
            ...
            //uint h = (uint)((x * x * x * p1) ^ (y * y * y * p2) ^ (z * z * z * p3));
            //uint h = (uint)((x * p1) ^ (y * p2) ^ (z * p3));

            h = (int)((uint)h % (uint)currentArraySize);
            //h = h % (uint)currentArraySize;
